...
exit event::wait(uint32_t mask, uint32_t *value, uint64_t time, error** _error) OS_NOEXCEPT
{
...
inline exit event::wait_from_isr(uint32_t mask, uint32_t *value, uint64_t time, error **error)
{
